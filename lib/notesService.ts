...
const generateSlugId = (series: string, section: string, title: string) => {
  const str = `${series}-${section}-${title}`;
...
tatvarthAdhyays.forEach(adhyay => {
  for (let i = 1; i <= adhyay.count; i++) {
    FALLBACK_NOTES.push({
      id: generateSlugId("Tatvarth Series", adhyay.name, `${adhyay.prefix} ${i}`),
