...
export function AceternityLamp({
  ...
  return (
    <div
      className={`relative flex min-h-[380px] md:min-h-[440px] flex-col items-center justify-center overflow-hidden w-full rounded-md z-0 ${className}`}
    >
      <div className="relative flex w-full flex-1 scale-y-125 items-center justify-center isolate z-0 ">
        ...
        <motion.div
          ...
          className="absolute inset-auto z-30 h-36 w-64 -translate-y-[6rem] rounded-full bg-amber-400 blur-2xl opacity-75"
        ></motion.div>
        ...
        <motion.div
          ...
          className="absolute inset-auto z-50 h-0.5 w-[30rem] -translate-y-[7rem] bg-amber-400 shadow-[0_0_25px_#f59e0b]"
        ></motion.div>

        <div className="absolute inset-auto z-40 h-44 w-full -translate-y-[12.5rem] bg-white dark:bg-black"></div>
      </div>

      {/* Content Placed Directly Under the Beam */}
      <div className="relative z-50 flex -translate-y-28 md:-translate-y-32 flex-col items-center px-5 text-center">
        {children}
      </div>
    </div>
  );
